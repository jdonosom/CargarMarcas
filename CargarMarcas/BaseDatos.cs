...
#nullable disable
    ...
    {
        string passPhrase = "UnaCadena";  // can be any string
        string saltValue = "UnaCadena";  // can be any string
        ...
        private void Configurar()
        {
            try
            {
                ...
                // var tab = GetProviderFactoryClasses(); 

                var appConfig = ConfigurationManager.OpenExeConfiguration(Assembly.GetExecutingAssembly().Location);

                string server = appConfig.AppSettings.Settings["SERVER"].Value;
                string user = Encriptacion.Decrypt(appConfig.AppSettings.Settings["USERNAME"].Value, passPhrase, saltValue, hashAlgorithm, passwordIterations, initVector, keySize);
                string password = Encriptacion.Decrypt(appConfig.AppSettings.Settings["PASSWORD"].Value, passPhrase, saltValue, hashAlgorithm, passwordIterations, initVector, keySize);
                string database = Encriptacion.Decrypt(appConfig.AppSettings.Settings["DATABASE"].Value, passPhrase, saltValue, hashAlgorithm, passwordIterations, initVector, keySize);
